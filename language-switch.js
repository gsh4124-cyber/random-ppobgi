(() => {
  ...
  function mount(){
    ...
    const box=document.createElement('label');box.id='languageSwitch';box.className='language-switch-one';box.setAttribute('aria-label','Language');
    const globe=document.createElement('span');globe.className='globe';globe.setAttribute('aria-hidden','true');globe.textContent='🌐';
    const select=document.createElement('select');select.id='languageSelect';select.setAttribute('aria-label','Language');
    Object.entries(langs).forEach(([code,label])=>{const o=document.createElement('option');o.value=code;o.textContent=label;select.append(o)});
