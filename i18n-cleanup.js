(() => {
  ...
  const jaExtra={'참여자는 1번부터 자동으로 만들어집니다.':'参加者は1番から自動で作成されます。','뽑을 인원':'抽選人数','결과':'結果','당첨':'当選','시작':'開始','주사위':'サイコロ','윷놀이':'ユンノリ','동전':'コイン','순서':'順番','팀 나누기':'チーム分け','주사위 굴리기':'サイコロを振る','최대 6개까지 한 번에 굴릴 수 있어요.':'最大6個まで一度に振れます。','개수':'個数','굴리기':'振る','🎲 굴려보세요':'🎲 振ってみよう','게임 진행에 바로 쓰는 5가지 도구':'ゲームですぐ使える5つのツール','입력한 내용은 이 브라우저에서만 사용돼요':'入力内容はこのブラウザ内でのみ使用されます。','8가지 방식으로 원하는 사람을 뽑아보세요':'8つの方法で好きな人を抽選できます','당첨 결과를 이어서 활용할 수 있어요':'抽選結果をそのまま活用できます','서비스 소개':'サービス紹介','사용방법':'使い方','개인정보처리방침':'プライバシー','이용약관':'利用規約','문의':'お問い合わせ','입력한 내용은 브라우저 밖으로 전송되지 않습니다.':'入力内容はブラウザの外部に送信されません。','다시 뽑기':'もう一度抽選','당첨자 제외하고 다시 뽑기':'当選者を除いて再抽選','바로 결과 보기':'結果をすぐ表示'};
  ...
  function isLanguageSelect(sel){if(sel.id==='finalLanguageSelect')return false;const texts=[...sel.options].map(o=>o.textContent.trim());return Object.values(names).filter(n=>texts.includes(n)).length>=3;}
  function removeOldLanguageUI(){
    document.querySelectorAll('#canonicalLanguageSwitch,#fullLanguageSelect,#canonicalLanguageSelect,[data-global-language-switch],.global-language-switch,.language-switcher,.language-selector').forEach(el=>{if(el.closest('#finalLanguageSwitch'))return;const p=el.parentElement;if(p&&p!==document.body&&p.children.length<=2)p.remove();else el.remove();});
    ...
    document.querySelectorAll('span,button,label,div').forEach(el=>{if(el.closest('#finalLanguageSwitch'))return;if(el.children.length===0&&el.textContent.trim()==='🌐')el.remove();});
  }
  function applyCore(){
    ...
    if(Object.keys(extra).length){const w=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT);const nodes=[];while(w.nextNode())nodes.push(w.currentNode);nodes.forEach(n=>{if(n.parentElement?.closest('#finalLanguageSwitch')||['SCRIPT','STYLE'].includes(n.parentElement?.tagName))return;const s=n.nodeValue.trim();if(extra[s])n.nodeValue=n.nodeValue.replace(s,extra[s]);});}
    document.documentElement.lang=lang;document.documentElement.dir=lang==='ar'?'rtl':'ltr';
  }
  function createLanguageUI(){
    document.querySelector('#finalLanguageSwitch')?.remove();
    const header=document.querySelector('header'); if(!header)return;
    const box=document.createElement('label');box.id='finalLanguageSwitch';box.setAttribute('aria-label','Language');box.style.cssText='display:flex;align-items:center;gap:6px;margin-left:auto;flex:0 0 auto';
    const globe=document.createElement('span');globe.textContent='🌐';
    const select=document.createElement('select');select.id='finalLanguageSelect';select.style.cssText='width:132px;height:38px;border:1px solid #e4ddd1;border-radius:11px;background:#fffdf8;padding:0 30px 0 10px;font:inherit;font-weight:800;color:#211d18';
  ...
  function finish(){removeOldLanguageUI();applyCore();createLanguageUI();}
  [0,120,400,900,1700,2400].forEach(ms=>setTimeout(finish,ms));
})();